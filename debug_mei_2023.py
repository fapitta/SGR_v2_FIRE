...
df_weights = load_sheet('cost_structure').T

target_year = 2023
calc_year = target_year - 2 # 2021

# Labor Rates (3 year avg: 2018 -> 2021)
base_year = calc_year - 3 # 2018
I_types = [c for c in df_inf.columns if '인건비' in c]
labor_rates = (df_inf.loc[calc_year, I_types] / df_inf.loc[base_year, I_types])**(1/3)

# Non-Labor Rates (1 year: 2020 -> 2021)
...
print(f"--- {target_year}년 MEI 산출 기초 수치 (T-2 = {calc_year}년 데이터 기준) ---")
print(f"\n[1] 인건비 증가율 (2018->2021 3년 연평균):")
for i, v in labor_rates.items():
    print(f"  {i}: {v:.6f}")

print(f"\n[2] 관리비/재료비 증가율 (2020->2021 1년):")
for i, v in raw_rates.items():
    print(f"  {i}: {v:.6f}")

print(f"\n[4] 시나리오 I1M1Z1 상세 계산 과정 ({target_year}년용):")
i_col, m_col, z_col = '인건비_1', '관리비_1', '재료비_1'
l_rate, m_rate, z_rate = labor_rates[i_col], raw_rates[m_col], raw_rates[z_col]

results = []
for t in hospital_types:
    w_l, w_m, w_z = df_weights.loc[t, '인건비'], df_weights.loc[t, '관리비'], df_weights.loc[t, '재료비']
    mei = (w_l * l_rate) + (w_m * m_rate) + (w_z * z_rate)
    results.append({
        '종별': t,
        'W_인건비': f"{w_l:.4f}", 'W_관리비': f"{w_m:.4f}", 'W_재료비': f"{w_z:.4f}",
        'L_Rate': f"{l_rate:.6f}", 'M_Rate': f"{m_rate:.6f}", 'Z_Rate': f"{z_rate:.6f}",
        'MEI_Index': f"{mei:.6f}", 'MEI_%': f"{(mei-1)*100:.4f}%"
    })
